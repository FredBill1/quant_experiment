...
def main() -> None:
    ...
    def qat() -> None:
        ...
        print("QAT")
        with SummaryWriter(log_dir=MODEL.parent / "dorefa") as writer:
        ...
        torch.save(model.state_dict(), MODEL.with_stem(MODEL.stem + "_dorefa"))

    def test_qat() -> None:
        model = create_model(from_pretrained=False, frozen=False)
        dorefa_prepare(model, inplace=True)
        model.load_state_dict(torch.load(MODEL.with_stem(MODEL.stem + "_dorefa")))
        model.to(device)
        test_loss, test_acc = evaluate(model, test_loader, criterion, device)
        print(f"{test_loss=} {test_acc=}")
