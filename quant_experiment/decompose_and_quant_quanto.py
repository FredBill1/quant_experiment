...
def main() -> None:
    ...
    for d_method in DECOMPOSE_METHODS:
        for d_ratio in DECOMPOSE_RATIOS if d_method is not None else [None]:
            decomposed_model_path = LOG_DIR / f"{d_method}_{d_ratio}.pth"
            if decomposed_model_path.exists() and all((LOG_DIR / f"{d_method}_{d_ratio}_{w}_{a}.pth").exists() for w, a in QUANT_CONFIGS):
                continue
            ...
            for w_dtype, a_dtype in QUANT_CONFIGS:
                quantized_model_path = LOG_DIR / f"{d_method}_{d_ratio}_{w_dtype}_{a_dtype}.pth"
